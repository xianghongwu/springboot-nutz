        ...
        if(importList!=null && importList.size()>0){
            for (StudentInfoDtoTemp studentInfoDtoTemp : importList) {
                if(studentInfoDtoTemp.getStudentId()==null || studentInfoDtoTemp.getStuName()==null || studentInfoDtoTemp.getUnionId()==null){
                    return new AjaxResult(AjaxCode.INNER_ERROR, "数据错误");
                }
            ...
            return new AjaxResult(AjaxCode.INNER_ERROR, "文件数据为空");
        }
        return new AjaxResult(AjaxCode.SUCCESS, AjaxCode.ADD_SUCCESS,"一共生成："+i+"张二维码；");
    }
}
